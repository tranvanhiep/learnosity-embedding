...
declare global {
  interface Window {
    LearnosityAuthor: {
      init: (request: unknown, selector: string, callbacks?: Record<string, unknown>) => void;
    };
  }
}
...
function ActivityContent() {
  ...
  const activityId = searchParams.get('id') || 'quickstart_examples_activity_template_001';
  ...
  useEffect(() => {
    async function fetchInitData() {
      try {
        const response = await fetch(`/api/activity?id=${encodeURIComponent(activityId)}`);
        if (!response.ok) {
  ...
  useEffect(() => {
    if (scriptLoaded && initData && window.LearnosityAuthor) {
      try {
        ...
        window.LearnosityAuthor.init(initRequest, '#learnosity-author', callbacks);
      } catch (err) {
  ...
  return (
    <>
      {initData && (
        <Script
          src={process.env.NEXT_PUBLIC_AUTHOR_API_URL || 'https://authorapi.learnosity.com/?latest-lts'}
      ...
      <div className="min-h-screen bg-gray-50 py-8 px-4">
        <div className="max-w-6xl mx-auto">
          ...
          <div className="bg-white rounded-lg shadow-lg p-8">
            <div className="mb-6">
              <h1 className="text-3xl font-bold text-gray-900 mb-2">Activity Editor</h1>
              <p className="text-gray-600 mb-2">
                Viewing activity: <code className="bg-gray-100 px-2 py-1 rounded">{activityId}</code>
              </p>
              <p className="text-sm text-gray-500">
                This activity uses the Learnosity Author API in activity_edit mode.
              </p>
            </div>

            <div id="learnosity-author" className="learnosity-container min-h-[600px]"></div>
          </div>
        </div>
      </div>
    </>
